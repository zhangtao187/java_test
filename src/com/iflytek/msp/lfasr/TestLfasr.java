...
{
	// 原始音频存放地址
	private static final String local_file = "D:\\dpv2t\\input\\test.mp3";
	/*
	 * 转写类型选择：标准版和电话版分别为：
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("德邦IT用户体验张涛coding");
			// 如果返回状态等于0，则任务处理成功
			if (resultMsg.getOk() == 0) {
				// 打印转写结果
				//System.out.println(resultMsg.getData());
				//下面过程主要的作用是将我们的日志信息打印至专门的txt文档中
				try {  
		            PrintStream out = System.out;  
		            PrintStream ps = new PrintStream("D://dpv2t//temp//test.txt");    
		            System.setOut(ps);  
		            System.out.println(resultMsg.getData());  
		            System.setOut(out);  
